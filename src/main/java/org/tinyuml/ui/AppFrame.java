...
implements EditorStateListener, AppCommandListener, SelectionListener {
  ...
  private JLabel elementCountLabel = new JLabel("    ");
  ...
  /**
   * Updates the current editor based on the selected tab.
   */
      ...
      if (comp instanceof JPanel) {
        ...
        for (Component child : panel.getComponents()) {
          if (child instanceof JScrollPane) {
            ...
            if (view instanceof DiagramEditor) {
              currentEditor = (DiagramEditor) view;
              break;
            }
          }
        }
  ...
  private void installStatusbar() {
    ...
    centerPanel.add(elementCountLabel, BorderLayout.CENTER);
  ...
  public void newModel() {
    if (canCreateNewModel()) {
      ...
      var factory = diagram.getElementFactory();
      var pkgNode = factory.createNode(org.tinyuml.model.ElementType.PACKAGE);
      pkgNode.setOrigin(50, 50);
      diagram.addChild(pkgNode);
      pkgNode.invalidate();
      var classNode = factory.createNode(org.tinyuml.model.ElementType.CLASS);
      classNode.setOrigin(250, 50);
      diagram.addChild(classNode);
      classNode.invalidate();
      var compNode = factory.createNode(org.tinyuml.model.ElementType.COMPONENT);
      compNode.setOrigin(450, 50);
      diagram.addChild(compNode);
      compNode.invalidate();

      try {
        umlModel.getClass().getMethod("addElement", Object.class).invoke(umlModel, pkgNode);
        umlModel.getClass().getMethod("addElement", Object.class).invoke(umlModel, classNode);
        umlModel.getClass().getMethod("addElement", Object.class).invoke(umlModel, compNode);
      ...
      if (currentEditor != null) {
        ...
        if (currentEditor.getDiagram() != null) {
          currentEditor.getDiagram().invalidate();
        }
      }
      updateElementCount();
    }
  ...
  /**
   * Updates the element count label in the status bar.
   */
  private void updateElementCount() {
    if (currentEditor == null) {
      elementCountLabel.setText("Total Items : 00; Package:00, Class:00; Component: 00");
      return;
    }
    
    StructureDiagram diagram = currentEditor.getDiagram();
    if (diagram == null) {
      elementCountLabel.setText("Total Items : 00; Package:00, Class:00; Component: 00");
      return;
    }
    ...
    int totalCount = packageCount + classCount + componentCount;
    String countText = String.format("Total Items : %02d; Package:%02d, Class:%02d; Component: %02d",
        totalCount, packageCount, classCount, componentCount);
    elementCountLabel.setText(countText);
  }
  ...
  public void copy(){
	  ...
	  //adicionalmente, hay que habilitar el boton PASTE!
